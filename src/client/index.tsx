...
export function apply(ctx: Context): void {
  ...
  try {
    ...
    ctx.effect(() => {
      ...
      // evaluation only.
      // Resolve the remote service through dshloader's stable client API
      // (window.__dshLoader__.services.get) instead of reading the cordis
      // context directly. Degrades to undefined when dshloader is absent.
      const remote = (typeof window !== 'undefined' ? window.__dshLoader__?.services?.get('remote') : undefined) as
