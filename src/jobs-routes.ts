 ...
 */
import type { Context, SidebarAgentsService, SidebarJobsService, SidebarSessionEvent, SidebarSessionStore } from './context-types.ts'
...
export function buildJobsApi(ctx: Context, outputLimit: number): SidebarJobsRoutes {
  const jobs = ctx.dshLoader.services.get<SidebarJobsService>('jobs')
  const agents = ctx.dshLoader.services.get<SidebarAgentsService>('agents')
  ...
  return {
    output(payload) {
      ...
      for (const event of sessions?.get(sessionId)?.events ?? []) {
        const trace = traceOf(event)
