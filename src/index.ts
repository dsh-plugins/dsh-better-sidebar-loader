...
import type { Context, SidebarHttpRequest, SidebarSessionStore, SidebarWebRuntime } from './context-types.ts'
import {
  Config,
  PrefsSchema,
...
import { isSettingsConflict, settingsNamespace } from './loader-compat.ts'
...
export const name = '@dsh-plugin/dsh-better-sidebar'
...
function sessionCwdOf(ctx: Context, sessionId: string, clientCwd?: string): string {
  const sessions = ctx.dshLoader.services.get<SidebarSessionStore>('sessions')
...
export function apply(ctx: Context, config?: SidebarConfig): void {
  ...
  const webRuntime = ctx.dshLoader.services.get<SidebarWebRuntime>('webRuntime')
  const fence = (req: SidebarHttpRequest): boolean =>
    webRuntime !== undefined ? isTrustedApiRequest(req, webRuntime.trustedHosts) : false
  ...
  if (scope !== undefined) {
    ...
    // the owner scope's get() — no describe round-trip needed.
    ...
    // namespace is read through the loader settings describe; an absent
    // namespace (no aionui installed) = not disabled.
    const externalDisable = (): boolean => {
      const descriptor = loaderSettings.describe({ redactSecrets: true })
    ...
    settingsFace = {
      ...
      update: async (patch, expectedRevision) => {
        const result = await loaderSettings.update(ns, patch as Record<string, unknown>, expectedRevision)
        if (result.ok === false) {
          // The loader returns a SettingsResult envelope instead of throwing:
          // translate a revision conflict back to the wire's 409 contract.
          if (result.code === 'settings-conflict') {
            throw new SidebarError('settings-conflict', result.message ?? 'settings namespace changed since it was read', 409)
          }
          throw new SidebarError('settings-rejected', result.message ?? 'settings update rejected', 400)
        }
        if (result.value?.revision !== undefined) revision = result.value.revision
        return { value: result.value?.value ?? scope.get(), revision }
      },
    }
  ...
  ctx.effect(() => ctx.dshLoader.web.register('/sidebar/api', async (req, res) => {
    if (!fence(req)) {
  ...
  ctx.effect(() => ctx.dshLoader.web.register('/sidebar/file', async (req, res) => {
      if (!fence(req)) {
        res.writeHead(403)
        res.end('forbidden')
        return
  ...
  ctx.effect(() => ctx.dshLoader.web.register('/sidebar/html', async (req, res) => {
      if (!fence(req)) {
        res.writeHead(403)
        res.end('forbidden')
        return
  ...
  // WS upgrade routes go through the loader's stable web API.
  ctx.effect(() => ctx.dshLoader.web.registerUpgrade({
