 ...
 */
import { defineTool } from './loader-compat.ts'
import type { ToolRunContext } from '@deepseek-ai/dsh-tools'
import type { ContentBlock } from '@deepseek-ai/dsh-llm'
import type { Agent } from '@deepseek-ai/dsh-agent'
import type { Context, SidebarToolsService } from './context-types.ts'
import {
  AgentPtyRegistry,
  ALLOWED_SIGNALS,
...
export function registerTools(
  ...
  const register = (tool: ReturnType<typeof defineTool>): void => {
    const tools = ctx.dshLoader.services.get<SidebarToolsService>('tools')
    if (tools !== undefined) disposers.push(tools.register(tool))
  }
