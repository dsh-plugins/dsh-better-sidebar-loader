...
/**
 * The dshloader stable settings API face (ctx.dshLoader.settings).
 * `register` returns the owner scope (like the official service's register);
 * `update` returns a SettingsResult envelope whose `value` carries the
 * fresh revision for revision-guarded writes. `describe` is filtered by
 * the loader's browser whitelist (see dshloader docs) — a plugin reading
 * its OWN non-whitelisted namespace must use the owner scope's get(),
 * not describe().
 */
export interface SidebarLoaderSettingsAPI {
  ...
  update(ns: string, section: object, expectedRevision?: number): Promise<{
    ok: boolean
    code?: string
    message?: string
    value?: { value?: unknown; revision?: number; ns?: string }
  }>
...
/**
 * The dshloader stable API face (`ctx.dshLoader`) — the ONLY host-side
 * entry point this transcription uses. Every dsh service is reached
 * through `services.get()`; web routes through `web.register()`; the
 * the settings namespace is registered through `ctx.dshLoader.settings.register()`
 * (namespace registration is not part of the loader's settings surface,
 * so it goes through the service escape hatch).
 */
export interface SidebarDshLoaderAPI {
  version: string
  dshVersion: string
  adapterVersion: string
  web: {
    register(prefix: string, handler: (req: SidebarHttpRequest, res: SidebarHttpResponse) => void | Promise<void>): () => void
    get(path: string, handler: (req: SidebarHttpRequest, res: SidebarHttpResponse) => void | Promise<void>): () => void
    post(path: string, handler: (req: SidebarHttpRequest, res: SidebarHttpResponse) => void | Promise<void>): () => void
    use(middleware: (req: SidebarHttpRequest, res: SidebarHttpResponse, next: () => void) => void): () => void
    registerUpgrade(route: {
      path: string
      handler: (req: SidebarHttpRequest, socket: SidebarUpgradeSocket, head: SidebarUpgradeHead) => void | Promise<void>
    }): () => void
  }
  settings: SidebarLoaderSettingsAPI
  services: {
...
declare module 'cordis' {
  interface Context {
    /**
     * The dshloader stable API. The host half reaches EVERY dsh service
     * (webServer / sessions / webRuntime / settings / tools / jobs / agents)
     * through this one seam — it never touches `ctx.webServer` /
     * `ctx.sessions` / `ctx.settings` / `ctx.tools` directly.
     */
    dshLoader: SidebarDshLoaderAPI
    /**
     * The client-side session list feed (client half only). The host half
     * reads the session store through `ctx.dshLoader.services.get('sessions')`.
     */
